...
fun AppCompatTextView.setResizableText(
    fullText: String,
    maxLines: Int,
    viewMore: Boolean,
    ...
    val suffixText =
        if (viewMore) " ${resources.getString(R.string.read_more)}" else " ${resources.getString(R.string.read_less)}"
    ...
    val lastHasNewLine = adjustedText.substring(
        textLayout.getLineStart(maxLines - 1), textLayout.getLineEnd(maxLines - 1)
    ).contains("\n")
    var linedText = if (lastHasNewLine) {
        ...
        adjustedText.take(textLayout.getLineStart(maxLines - 1)) + adjustedText.substring(
            textLayout.getLineStart(maxLines - 1), textLayout.getLineEnd(maxLines - 1)
        ).replace("\n", lineOfSpaces) + adjustedText.substring(textLayout.getLineEnd(maxLines - 1))
    } else {
        adjustedText
    }
...
private fun AppCompatTextView.addClickablePartTextResizable(
    fullText: String,
    maxLines: Int,
