...
class AppointmentRepository @Inject constructor() {
    ...
    suspend fun getAppointmentsProgressList(
        date: Date, userId: String, firestore: FirebaseFirestore
    ): ApiResponse<List<AppointmentModel>> {
        return try {
            val nextDate = Calendar.getInstance()
            nextDate.time = date
            ...
            val response =
                firestore.collection(TABLE_APPOINTMENT).whereEqualTo(FIELD_DOCTOR_ID, userId)
                    ...
                    .whereGreaterThanOrEqualTo(FIELD_SELECTED_DATE, date)
                    .whereLessThanOrEqualTo(FIELD_SELECTED_DATE, nextDate.time).get().await()
            val appointmentsList = arrayListOf<AppointmentModel>()
            for (document: DocumentSnapshot in response.documents) {
                val user = document.toObject(AppointmentModel::class.java)
                user?.id = document.id
    ...
    suspend fun updateAppointmentData(
        ...
        return try {
            val response = fireStore.collection(TABLE_APPOINTMENT)
                .whereEqualTo(FIELD_USER_ID, requestModel.userId).get().await()

            val updateUserResponse =
                fireStore.collection(TABLE_APPOINTMENT).document(response.documents[0].id)
    ...
    suspend fun getBookAppointmentDetailsList(
        selectedDate: Date, userId: String, fireStore: FirebaseFirestore
    ): ApiResponse<List<AppointmentModel>> {
        return try {
            val nextDate = Calendar.getInstance()
